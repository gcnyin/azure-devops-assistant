...
export function BoardView({
  ...
  useEffect(() => {
    if (fixesMutation.isSuccess) navigate("/fixes");
  }, [fixesMutation.isSuccess, navigate]);
  ...
  if (isError) {
    return (
      <div className="text-center py-16 text-accent-tomato">
        Failed to load board data. The server may be unavailable.
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 mb-6">
        <span className="text-sm text-ink-mute mr-1">View:</span>
        <Button
        ...
        >All</Button>
        <Button
          variant={view === "me" ? "secondary" : "ghost"}
          size="sm"
          className="rounded-md"
          onClick={() => setView("me")}
        >Me</Button>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap max-md:flex-col max-md:items-stretch max-md:gap-2">
        ...
        <div className="flex items-center gap-1 flex-wrap max-md:overflow-x-auto max-md:flex-nowrap max-md:pb-1">
          <Button
            variant={stateFilter === "all" && !diffFilter ? "secondary" : "ghost"}
          ...
          >Open <span className="text-ink-faint ml-1 text-xs">{incCount}</span></Button>
          <Button
            variant={stateFilter === "done" ? "secondary" : "ghost"}
            size="sm" className="rounded-md"
            onClick={() => updateParam("state", "done")}
          >Done <span className="text-ink-faint ml-1 text-xs">{compCount}</span></Button>
          {bugCount > 0 && (
            <Button
              variant={stateFilter === "bug" ? "secondary" : "ghost"}
              size="sm" className="rounded-md"
              onClick={() => updateParam("state", "bug")}
            >Bug <span className="text-ink-faint ml-1 text-xs">{bugCount}</span></Button>
          )}
          {Object.entries(stateMap).sort(([a], [b]) => a.localeCompare(b)).map(([state, count]) => (
            <Button
              key={state}
              variant={stateFilter === state.toLowerCase() ? "secondary" : "ghost"}
              size="sm" className="rounded-md"
              onClick={() => updateParam("state", state.toLowerCase())}
            >{state} <span className="text-ink-faint ml-1 text-xs">{count}</span></Button>
          ))}
        </div>

        <div className="flex-1 max-md:hidden" />
        <span className="text-[13px] text-ink-mute whitespace-nowrap max-md:self-start">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3 max-md:flex-col max-md:items-start max-md:gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {nn > 0 && (
            <Badge
              variant="outline"
              className={`diff-tag new cursor-pointer hover:brightness-110 select-none ${diffFilter === "new" ? "outline-2 outline-offset-1 outline-primary" : ""}`}
              onClick={() => updateParam("diff", diffFilter === "new" ? "" : "new")}
            >+{nn} New</Badge>
          )}
          {nc > 0 && (
            <Badge
              variant="outline"
              className={`diff-tag changed cursor-pointer hover:brightness-110 select-none ${diffFilter === "changed" ? "outline-2 outline-offset-1 outline-accent-yellow" : ""}`}
              onClick={() => updateParam("diff", diffFilter === "changed" ? "" : "changed")}
            >~{nc} Changed</Badge>
          )}
          {ng > 0 && (
            <Badge
              variant="outline"
              className={`diff-tag gone cursor-pointer hover:brightness-110 select-none ${diffFilter === "gone" ? "outline-2 outline-offset-1 outline-accent-tomato" : ""}`}
              onClick={() => updateParam("diff", diffFilter === "gone" ? "" : "gone")}
            >-{ng} Gone</Badge>
          )}
          {newBugs.length > 0 && (
            <Button
              variant="secondary" size="sm"
              onClick={() => fixesMutation.mutate()}
              disabled={fixesMutation.isPending}
            >
              {fixesMutation.isPending ? "Generating..." : `Generate AI Fixes (${newBugs.length} new bugs)`}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="table-wrap">
        {filteredItems.length === 0 && !data ? (
          ...
          <div className="flex flex-col items-center py-24 text-ink-mute">
            ...
            <div className="text-base text-ink-secondary font-medium mb-2">No results</div>
